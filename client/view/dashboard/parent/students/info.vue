      ...
      <v-flex slot="item" slot-scope="props" xs12 md6 pa-2>
        <v-card>
          <v-card-title class="secondary justify-center display-1">
            <h4 class="white--text">{{ `${props.item.firstname} ${props.item.lastname}` }}</h4>
          </v-card-title>
          <v-divider></v-divider>
          <v-container fluid class="scrollBar">
            <v-layout row wrap align-center>
              ...
              <v-flex xs5 pa-2 class="text-xs-right">
                {{ showDate(props.item.date_birthday) }}
              </v-flex>
              <v-flex xs7 pa-2>
                Email
              </v-flex>
              ...
              <v-flex xs5 pa-2 class="text-xs-right">
                {{ showDate(props.item.date_enrolled) }}
              </v-flex>
              <v-flex xs7 pa-2>
                Parent / Guardian
              </v-flex>
              <v-flex xs5 pa-2 class="text-xs-right">
                {{ `${props.item.resp_firstname} ${props.item.resp_lastname}` }}
              </v-flex>
              <v-flex xs7 pa-2>
                Parent / Guardian Email
              </v-flex>
              <v-flex xs5 pa-2 class="text-xs-right">
                {{ props.item.resp_email}}
              </v-flex>
              <v-flex xs12 px-3 pt-3 class="text-xs-center">
                <v-tooltip left>
                  <v-btn block color="green" class="ma-0 pa-0 white--text" @click="editItem(props.item)" slot="activator">
